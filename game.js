...
document.getElementById('mapBtn').onclick=window.openMap;
document.getElementById('modeBtn').onclick=()=>show('⚔️ PILIH MODE','<p>Pilih arena sebelum battle berikutnya.</p>'+Object.entries(modes).map(([id,m])=>'<div class="modeCard"><b>'+m.name+'</b><small>'+m.desc+' • Reward '+m.reward+' 🪙</small><button onclick="chooseMode(\''+id+'\')">'+(mode===id?'✓ DIPILIH':'PILIH MODE')+'</button></div>').join(''));
